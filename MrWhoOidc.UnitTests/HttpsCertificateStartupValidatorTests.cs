...
public class HttpsCertificateStartupValidatorTests
{
    ...
    [TestMethod]
    public void Returns_False_When_Certificate_Directory_Is_Missing()
    {
        var config = new ConfigurationBuilder()
            .AddInMemoryCollection(new Dictionary<string, string?>
            {
                ["ASPNETCORE_ENVIRONMENT"] = "Development",
                ["ASPNETCORE_URLS"] = "https://+:8443;http://+:8080",
                ["Kestrel:Certificates:Default:Path"] = Path.Combine(Path.GetTempPath(), Guid.NewGuid().ToString(), "cert.pfx")
            })
            .Build();
