...
public sealed class SecurityBoundaryTests
{
    ...
    public async Task Security_Cross_Realm_Role_Leakage_Prevented()
    {
        ...
        Assert.AreEqual(1, userRoles.Count, "Should only have 1 role from Realm1");
        Assert.IsTrue(userRoles.Contains("Admin"), "Should have Admin role from Realm1");
        Assert.IsFalse(userRoles.Contains("SuperAdmin"), "Should NOT have SuperAdmin role from Realm2");
    }
    ...
    public async Task Security_Scope_Escalation_Prevented()
    {
        ...
        Assert.IsTrue(unauthorizedScopes.Length > 0, "Unauthorized scopes should be detected");
    ...
    public async Task Security_Token_Audience_Isolation_Between_Clients()
    {
        ...
        Assert.AreEqual(1, client1Tokens.Count, "Client1 should have 1 token");
        Assert.AreEqual("api-for-client1", client1Tokens[0].Audience);
        
        Assert.AreEqual(1, client2Tokens.Count, "Client2 should have 1 token");
    ...
    public void Security_Client_Secret_Never_In_Logs()
    {
        ...
        Assert.IsFalse(hashedSecret.Contains(plainSecret), "Hashed secret should NOT contain plain text");
        Assert.IsTrue(hashedSecret.Length > plainSecret.Length, "Hash should be longer than plain secret");
        Assert.IsTrue(hashedSecret.StartsWith("$argon2"), "Should use Argon2");
