...
| Token endpoint (grant types incl. refresh, client_credentials, device_code, token_exchange) | ✅ Implemented | `MrWhoOidc.WebAuth/Handlers/TokenHandler.cs` and grant handlers | `MrWhoOidc.UnitTests/*Token*` | DPoP, token exchange and rate limits implemented; auth-code/refresh now enforce `audience`/`resource` conflict checks and absolute-URI validation. |
...
| Claims request parameter | ✅ Implemented | `AuthorizeRequest` handling, persisted metadata, `UserInfoHandler` filtering, token auth-code exchange handling | Unit tests validate claims filtering and token endpoint pass-through | Token endpoint now accepts `claims` for `authorization_code` with normalization/validation. |
...
| mTLS (RFC 8705) | ✅ Implemented | Discovery advertising: `mtls_endpoint_aliases` in `DiscoveryHandler.cs`; token endpoint mTLS auth in `ClientAuthenticationService`; mTLS checks in Revocation/Token | Unit tests and integration tests for mTLS auth (see: `TokenHandlerTests.cs::Token_Mtls_ClientCredentials_*`, `ClientAuthenticationServiceTests.cs`) | Token endpoint now accepts mTLS client auth consistently across token grants when client thumbprints match; CB-TLS issuance remains on supported issuance paths. |
