...
| mTLS (RFC 8705) | ✅ Implemented | Discovery advertising: `mtls_endpoint_aliases` in `DiscoveryHandler.cs`; mTLS auth checks in Revocation/Token | Unit tests and integration tests for mTLS auth (see: `TokenHandlerTests.cs::Token_Mtls_ClientCredentials_*`) | Discovery advertises `tls_client_certificate_bound_access_tokens=true`; server issues certificate-bound access tokens (CB‑TLS) for mTLS client_credentials. |
