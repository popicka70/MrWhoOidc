...
- Hosted conformance plan id: `h2MKe6xNghheb`
- Hosted plan URL: `https://www.certification.openid.net/plan-detail.html?plan=h2MKe6xNghheb`
- Export bundle: `tools/certification/.generated/exports/oidcc-basic-certification-test-plan-discovery-dynamic_client-h2MKe6xNghheb-20-Apr-2026.zip`
...
- Suite alias used for the hosted run: `mrwhooidc-0420135217-2a40b2`
...
Official runner summary for plan `h2MKe6xNghheb`:
...
1. `oidcc-prompt-login`
   - Log URL: `https://www.certification.openid.net/log-detail.html?log=WhOpQE2zpBW6uCe`
...
2. `oidcc-max-age-1`
   - Log URL: `https://www.certification.openid.net/log-detail.html?log=vVuWyW0I9f9i4vc`
...
3. `oidcc-ensure-registered-redirect-uri`
   - Log URL: `https://www.certification.openid.net/log-detail.html?log=xlNtouaS7VJO2mi`
...
> Submitting OpenID Connect Basic OP certification for `MrWhoOidc.WebAuth` using discovery metadata and dynamic client registration. The deployment under test is `https://mrwho.onrender.com/t/default` running version `1.0.0` in Production. Hosted conformance plan `h2MKe6xNghheb` completed on 2026-04-20 with `2068 successes, 0 failures, and 0 warnings`. The previously investigated `oidcc-scope-profile`, `oidcc-codereuse`, and `oidcc-codereuse-30seconds` modules all passed. The only remaining non-pass outcomes are manual `REVIEW` items for `oidcc-prompt-login`, `oidcc-max-age-1`, and `oidcc-ensure-registered-redirect-uri`; the requested screenshots will be uploaded with the submission. Expected skips correspond to intentionally unsupported optional features: unsigned ID tokens, `address`/`phone` scope coverage, and unsigned request object / unsigned `request_uri` coverage.
...
- Include the hosted plan id `h2MKe6xNghheb` in any reviewer notes or correspondence.
