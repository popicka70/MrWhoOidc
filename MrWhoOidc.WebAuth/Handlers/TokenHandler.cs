...
public sealed class TokenHandler(OidcOptions options, ITokenService tokens, IClientStore clients, OidcMetrics metrics) : ITokenHandler
{
    public async Task<IResult> HandleAsync(HttpContext http)
    {
        ...
        metrics.TokenRequests.Add(1);
        try
        {
            if (!http.Request.HasFormContentType)
            {
                metrics.TokenFailures.Add(1);
            ...
            if (string.Equals(grantType, "authorization_code", StringComparison.Ordinal))
            {
                ...
                if (ok) metrics.TokenSuccess.Add(1); else metrics.TokenFailures.Add(1);
        ...
        {
            sw.Stop();
            metrics.TokenDurationMs.Record(sw.Elapsed.TotalMilliseconds);
        }
    }
