...
{
    public sealed record Item(Guid Id, string Name, string Display, string? LogoUrl, bool HasLogoData, bool IsRecommended = false, string? ButtonBackgroundColor = null, string? ButtonTextColor = null);
    ...
    public async Task<IActionResult> OnGetAsync()
    {
        ...
        var providerLinks = await db.ClientIdentityProviders.AsNoTracking()
            ...
            .OrderBy(x => x.m.Order)
            .Select(x => new Item(x.p.Id, x.p.Name, x.p.DisplayName ?? x.p.Name, x.p.LogoUrl, x.p.LogoData != null, false, x.p.ButtonBackgroundColor, x.p.ButtonTextColor))
            .ToListAsync();
