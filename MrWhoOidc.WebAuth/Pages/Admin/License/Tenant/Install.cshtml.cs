...
/// <summary>
/// Tenant license installation page - allows installing/updating licenses for the current tenant.
/// Validates that only tenant-scoped licenses can be installed here.
/// </summary>
...
public class InstallModel : LicensePageModelBase
{
    ...
    public async Task<IActionResult> OnPostValidateAsync(CancellationToken cancellationToken)
    {
            ...
            if (ValidationPreview.License is not null)
            {
                ...
                if (string.Equals(ValidationPreview.License.Scope, "platform", StringComparison.OrdinalIgnoreCase))
                {
                    ValidationError = new LicenseValidationErrorDto(
                        "invalid_scope",
                        "This is a platform license, not a tenant license. Use the platform license page to install platform licenses.",
        ...
        {
            _logger.LogError(ex, "Failed to validate tenant license key for tenant {TenantId}", TenantId);
    ...
    public async Task<IActionResult> OnPostInstallAsync(CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            TempData["SuccessMessage"] = "Tenant license installed successfully.";
            return RedirectToPage("Index");
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Failed to install tenant license for tenant {TenantId}", TenantId);
    ...
    private void ApplyCurrentTenant()
    {
