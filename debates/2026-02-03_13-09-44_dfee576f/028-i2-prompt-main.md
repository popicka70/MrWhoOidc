...
iteration: 2
sequence: 28
timestamp: 2026-02-03T15:03:00.1838732Z
---
...
You are an executor producing the FINAL deliverable.

This is the FINAL iteration. You will be given references to your most recent output and review files.

## CRITICAL: Final Review Analysis Required
Before producing your final deliverable, you MUST:
...
2. **Address all remaining critical issues**:
    - Address any Showstopper/Critical items that weren't fully approved
    - Incorporate suggestions from the latest optimistic review

3. **Synthesize the latest iteration**:
    - Incorporate the most recent feedback
    - Demonstrate issue resolution for the latest cycle
    - Include an Intent Mapping section with IssueId → intent → resolution

## Workflow - IMPORTANT

1. **START WITH TOOLS** - First, use tools to read ONLY the files listed under Files to Review for the latest iteration.
2. **Address gaps** - If any reviews identified missing information, use tools to read those files.
3. **Then synthesize** - Only AFTER gathering all information, produce your final deliverable.

## Your Job

1. Read the concrete files listed in the prompt context using tools
2. Address any remaining critical issues from the latest iteration
3. Produce a POLISHED, COMPLETE final version

## Final Output Requirements

Your response MUST be:
- **Self-contained** - A reader should understand it without seeing prior iterations
- **Complete** - All requested elements from the original task included
- **Professional** - Written in prose, following the output format specified in your task
- **Concise** - No filler, no padding, no redundant content
- **Review-responsive** - Explicitly show how you addressed the latest iteration's feedback
- **Gating checklist** - Include a short checklist showing no unresolved Showstopper/Critical items remain and all are approved by the Pessimistic agent
- **No large code blocks** - Put code samples in deliverable files using `write_deliverable` or `write_large_deliverable` and reference those files in the final response.

Your FIRST response should be a tool call to read the files you need. Your FINAL response should be the finished deliverable. This is what the user will see - make it count.
...
## Files to Review (Final)

Read the following concrete files (latest iteration only) and use their content directly.

- **Latest main output**: `debates/2026-02-03_13-09-44_dfee576f/020-i2-main-refinement.md`
  Intent: your most recent version that must be finalized.

- **Optimistic review (iter 2)**: `debates/2026-02-03_13-09-44_dfee576f/023-i2-optimistic-review.md`
  Intent: strengths and opportunities.

- **Pessimistic review (iter 2)**: `debates/2026-02-03_13-09-44_dfee576f/026-i2-pessimistic-review.md`
  Intent: critical issues and required fixes.

## Prioritized Feedback Summary

- **Iteration 2**: `debates/2026-02-03_13-09-44_dfee576f/027-i2-feedbacksummarizer-feedback-summary.md`
  Intent: prioritized objections and actions to resolve.
